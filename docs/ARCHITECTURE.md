...
## 页面级 UAT 补充（2026-03-27）

- 当前 `apps/api` 的 `booking-rules` 控制器只落了 `GET /api/v1/staff/booking-rules`，缺少冻结契约要求的 `PUT` 写接口；这会在真实页面环境中稳定表现为店员规则保存 `404`。
- 当前 `gallery` 模块为纯数据库读取；若 MySQL 中 `gallery_items` 为空，首页会进入“返图内容整理中”的空态。因此当前主线除代码可用外，还需要最小可见返图基线数据，避免页面 UAT 看起来像展示能力失效。
- 店员月历当前已具备“常驻 + 数量聚合”基础，但还未满足最新店员工作台诉求：日期格需尽量显示顾客识别信息，选中日期后需输出按时间段排序的具体预约明细，而不只是一句总数提示。
- 顾客预约“未来日期只显示今天”的问题在真实页面环境中重新出现；在 `booking-rules` 写链路恢复前，前后端都不能只凭静态代码判断该问题已关闭，需结合真实 `availability.dateOptions` 返回和当前规则数据再次回归。
