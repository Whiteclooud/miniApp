...
- DEV-001：architect 已补 `infra/compose/docker-compose.api.yml` 与 `docs/API_ENV_AND_CUTOVER.md`，明确 MySQL 本地环境、`apps/server` / `apps/api` 并行方式、切换步骤与回滚手册。当前剩余动作是基于该环境手册做 `apps/api` 的 `migrate deploy + /health` 运行级验证。
