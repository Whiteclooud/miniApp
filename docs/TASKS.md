...
- 旧 `apps/server` 与围绕并行切换产生的过渡文档已完成主仓退场清理
- 本轮主线收口已完成，当前重点从“旧基线退场”切换为“页面级 UAT + 新主线任务整理”
- 后续任务优先围绕页面验收、文档收口、`apps/api` 稳态建设推进

## 当前主要 TODO（2026-03-24 夜间整理）

### P0：页面级 UAT（Lan 可抽空执行）
- 按当前 `apps/api` 唯一基线跑一轮真实页面验收
- 重点核对：首页返图封面 -> 详情多图、顾客预约未来日期窗口、不可预约原因展示、我的预约回查、店员规则结构化配置、店员月历常驻、店员审核/驳回/顾客侧结果回查
- 若页面通过，则本轮“新基线收口”可正式记为完成里程碑

### P1：主线文档收口
- 把 `ARCHITECTURE.md`、`UAT_RESULTS.md`、`TASKS.md` 里仍残留的“待删除 apps/server / 并行切流阶段”描述继续清理到当前事实
- 把历史任务保留为审计记录，但补一层“当前有效任务视图”，避免后续继续在旧任务堆里找主线

### P1：新主线稳态检查
- 确认 README、根脚本、开发说明全部以 `apps/api + apps/weapp` 为准
- 补一版当前主线可直接执行的页面 UAT 清单，替代已删除的旧 cutover checklist

### P2：下一阶段任务（进入正常维护/演进）
- frontend：在不破坏现有页面的前提下，继续做 services / types / TS 边界收口
