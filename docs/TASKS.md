...
- 运行级验证更新（2026-03-22 10:41 Asia/Shanghai）：architect 已在当前主仓 `apps/api` 基于真实 MySQL 数据执行 review 写链路 smoke：先直写 1 条 `pending` 预约并成功调用 `POST /api/v1/staff/appointments/:id/review -> approved`，随后对同一记录再次 `PATCH review` 收到 `409 + APPOINTMENT_ALREADY_REVIEWED`；再直写“1 条已 `approved` + 1 条同 slot 的 `pending`”组合，调用 `PATCH review -> approved` 收到 `409 + SLOT_OCCUPIED`。说明当前 `apps/api` 的店员审核写接口已在运行级覆盖 happy-path、重复审核拦截与 slot 冲突三类关键分支。
- 当前下一步：继续把本轮 smoke 结果补写到 `docs/API_PARALLEL_RUNBOOK.md`，并评估是否需要补一份可复用的脚本化 smoke 入口；在 availability/create appointment 等主链路尚未迁入前，仍不把 `apps/api` 视为可切流基线。
