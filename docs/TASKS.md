...
| BE-018 | backend | 在 `apps/api` 迁入顾客侧 create appointment 写接口并对齐冻结契约 | `docs/API.md` create appointment 契约、现有 `apps/server` appointments 写逻辑、当前 `apps/api` Prisma 模型 | `apps/api` customer appointment create 模块/路由、`POST /api/v1/appointments`、最小自测结论 | BE-011, BE-013 | 新 `apps/api` 可按 `X-Customer-OpenId` 创建 `pending` 预约，兼容 `appointmentDate/date`，校验规则外日期/非法 slot，并保持冻结返回结构 | 若身份头、slot 校验或状态默认值偏差，会直接破坏顾客申请主链路 |
| BE-019 | backend | 在 `apps/api` 迁入顾客侧 availability 读接口并对齐冻结契约 | `docs/API.md` availability 契约、现有 `apps/server` availability 逻辑、当前 `apps/api` booking-rules/appointments 读模型 | `apps/api` availability 模块/路由、`GET /api/v1/availability`、最小自测结论 | BE-011, BE-013, BE-015 | 新 `apps/api` 可按 `date` 返回全部应展示时段，并附带 `status/reasonCode/reasonText`，受 booking-rules 与 approved 占用影响 | 若 disabled 原因、日期边界或占用语义漂移，会直接破坏顾客预约页判断 |
...
- 当前下一步：Phase 0/1 已将下一批明确可执行任务正式化为 `BE-018`（顾客侧创建预约写接口）与 `BE-019`（availability 读接口）。执行顺序默认先 `BE-018` 后 `BE-019`，以便先补“创建预约 -> 店员审核”跨接口闭环，再补顾客侧时段选择体验；在这些核心顾客侧主链路尚未迁入前，仍不把 `apps/api` 视为可切流基线。
